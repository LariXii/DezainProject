...
var Jeu = {
    ...
    deconnecterJoueur : function(joueur){
        ...
        console.log("Joueurs en jeu",Jeu.manche.t_doisJouer);
    },
    manche : {
        t_doisJouer : [],
        nonDessinateur : [],
        Start: function(t_nomJoueurs){
            ...
            io.sockets.emit('startManche',Jeu.nbMancheJouer,Jeu.nbManche);
            this.lancerTour();
        },
        lancerTour(){
            this.nonDessinateur = [];
            ...
            Jeu.tour.setTour(this.nonDessinateur);
        },
        fin(){
            Jeu.enJeu = false;
    ...
    tour : {
        ...
        setTour(joueursNonDrawer){
            ...
            for(var j in Jeu.ensembleJoueurs){
                ...
                Jeu.ensembleJoueurs[j].dessinateur = false;
            }
            ...
            for(var i in Jeu.ensembleJoueurs){
            	if(!Jeu.ensembleJoueurs[i].dessinateur){
            		clients[Jeu.ensembleJoueurs[i].nom].emit('joueur');
            }
            }
...
io.on('connection', function (socket) {
   // io.sockets.emit('test');
    ...
    socket.on('canvas',function(img/*curr,x,y*/){
        for(var i in Jeu.ensembleJoueurs){
        if(!Jeu.ensembleJoueurs[i].dessinateur){
        	clients[Jeu.ensembleJoueurs[i].nom].emit('canvas',img/*curr,x,y*/);
        }
        }
    });
    /**
    ...
    socket.on("disconnect", function(reason) {
        ...
        if (currentID) {
            ...
            // envoi de la nouvelle liste pour mise à jour
            socket.broadcast.emit("liste", Jeu.ensembleJoueurs,Jeu.t_nomJoueurs);
            console.log("Client déconnecté\nIl reste "+Jeu.nbJoueur+" joueur(s)");
            if(Jeu.nbJoueur === 0){
                Jeu.chrono.Stop();
                Jeu.enJeu = false;
                Jeu.finManche = false;
